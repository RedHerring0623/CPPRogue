...
| 组件 | 职责 |
|---|---|
| `GameBootstrap` | 演示入口：`RuntimeInitializeOnLoadMethod` 在进 Play Mode 时自动搭好一切（不改场景文件）。正式场景就绪后移除，组件挂进场景 |
...
| `PauseController` + `EditorPanel` | **ESC 暂停/继续**：暂停时 timeScale=0 冻结世界并弹出拼装编辑面板（拖拽插入/移动、右键删除、双击改参数——DemoUI 的 Unity 版）；继续时热替换 Routine |
...
**按 ESC**：世界冻结，弹出拼装面板自由编辑；再按 ESC 用新 Routine 继续跑（变量保留）。
...
- 子弹无命中判定（无敌人）——接 `CircleOverlap` + 木桩怪
- `GameBootstrap` 全代码搭建是演示手段——正式版做成场景 + Prefab
- 玩家无敌（无受击）——接怪物 AI（怪也是 Routine）
- UI 无撤销（编辑误删只能重新拼）
